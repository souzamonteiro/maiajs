...
test('golden: invocation bridge helpers validate call-shape and select function id', () => {
  ...
  assert.match(cpp, /static int __maia_runtime_lambda_invoke_function_id\(void\* lambda_value, int argc, int async_call\) \{[\s\S]*if \(!__maia_runtime_lambda_can_invoke\(lambda_value, argc, async_call\)\) \{ return 0; \}[\s\S]*int function_id = __maia_runtime_lambda_select_function_id\(lambda_value, argc, async_call\);[\s\S]*if \(!function_id\) \{ return 0; \}[\s\S]*switch \(function_id\) \{[\s\S]*default:[\s\S]*return 0;[\s\S]*\}[\s\S]*\}/,
    'invocation bridge dispatch stub should source its switch key from the selector helper and preserve strict default-fail behavior');
  assert.match(cpp, /switch \(function_id\) \{[\s\S]*case 1001001:[\s\S]*if \(__maia_runtime_lambda_get_arity\(lambda_value\) != 1\) \{ return 0; \}[\s\S]*if \(__maia_runtime_lambda_get_is_async\(lambda_value\) != 1\) \{ return 0; \}[\s\S]*return -\(\(__maia_runtime_lambda_get_capture_at\(lambda_value, 0\) \* 1\) \+ \(__maia_runtime_lambda_get_capture_at\(lambda_value, 1\) \* 2\) \+ \(__maia_runtime_lambda_get_capture_at\(lambda_value, 2\) \* 3\) \+ \(__maia_runtime_lambda_get_capture_at\(lambda_value, 3\) \* 4\) \+ \(__maia_runtime_lambda_get_arity\(lambda_value\) \* 10\) \+ argc \+ 1 \+ 10 \+ 100 \+ \(__maia_runtime_lambda_get_capture_count\(lambda_value\) \* 1000\)\);/,
    'invocation bridge dispatch scaffold should materialize known case labels with metadata guards and function-id/arity/capture-family/arity-family/exact-capture-count-differentiated weighted async return behavior');
});
