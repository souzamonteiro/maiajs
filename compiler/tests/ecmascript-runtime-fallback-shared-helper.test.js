...
test('runtime fallback dedup: capture-aware lambda fallback reuses shared lambda payload allocator', () => {
  ...
  assert.match(cpp, /static int __maia_runtime_lambda_known_case_token\(void\* lambda_value, int function_id\) \{[\s\S]*int expected_capture_count = normalized_id % 1000;[\s\S]*int capture_family = expected_capture_count > 4 \? 40 : \(expected_capture_count > 1 \? 20 : 10\);[\s\S]*int runtime_arity = __maia_runtime_lambda_get_arity\(lambda_value\);[\s\S]*int runtime_capture_count = __maia_runtime_lambda_get_capture_count\(lambda_value\);[\s\S]*int token = \(runtime_arity \* 10\) \+ \(expected_capture_count % 10\);[\s\S]*token \+= capture_family;[\s\S]*token \+= arity_family;[\s\S]*token \+= \(runtime_capture_count \* 1000\);[\s\S]*return token;[\s\S]*\}/,
    'runtime-facing invocation bridge helper surface should include a dedicated known-case token helper');
  assert.match(cpp, /static int __maia_runtime_lambda_known_case_polarity\(int function_id\) \{[\s\S]*if \(function_id <= 0\) \{ return 0; \}[\s\S]*return function_id >= 1000000 \? -1 : 1;[\s\S]*\}/,
    'runtime-facing invocation bridge helper surface should include a dedicated known-case polarity helper');
  assert.match(cpp, /static int __maia_runtime_lambda_known_case_weighted_capture_value\(void\* lambda_value, int function_id\) \{[\s\S]*int capture_count = __maia_runtime_lambda_get_capture_count\(lambda_value\);[\s\S]*int weighted = 0;[\s\S]*weighted \+= \(__maia_runtime_lambda_get_capture_at\(lambda_value, 0\) \* 1\);[\s\S]*weighted \+= \(__maia_runtime_lambda_get_capture_at\(lambda_value, 1\) \* 2\);[\s\S]*weighted \+= \(__maia_runtime_lambda_get_capture_at\(lambda_value, 2\) \* 3\);[\s\S]*weighted \+= \(__maia_runtime_lambda_get_capture_at\(lambda_value, 3\) \* 4\);[\s\S]*if \(capture_count > 4\) \{[\s\S]*weighted \+= __maia_runtime_lambda_get_capture_at\(lambda_value, 4\);[\s\S]*\}[\s\S]*return weighted;[\s\S]*\}/,
    'runtime-facing invocation bridge helper surface should include a dedicated weighted-capture helper');
  assert.match(cpp, /static int __maia_runtime_lambda_known_case_matches_function_id\(void\* lambda_value, int function_id\) \{[\s\S]*int expected_is_async = function_id >= 1000000 \? 1 : 0;[\s\S]*int normalized_id = expected_is_async \? \(function_id - 1000000\) : function_id;[\s\S]*int expected_arity = normalized_id \/ 1000;[\s\S]*if \(__maia_runtime_lambda_get_arity\(lambda_value\) != expected_arity\) \{ return 0; \}[\s\S]*if \(__maia_runtime_lambda_get_is_async\(lambda_value\) != expected_is_async\) \{ return 0; \}[\s\S]*return 1;[\s\S]*\}/,
    'runtime-facing invocation bridge helper surface should include a dedicated known-case metadata-match helper');
  assert.match(cpp, /static int __maia_runtime_lambda_has_known_case\(int function_id\) \{[\s\S]*return function_id > 0 \? 1 : 0;[\s\S]*\}/,
    'runtime-facing invocation bridge helper surface should include a dedicated known-case membership helper');
  assert.match(cpp, /static int __maia_runtime_lambda_invoke_known_case\(void\* lambda_value, int function_id, int argc\) \{[\s\S]*if \(!__maia_runtime_lambda_has_known_case\(function_id\)\) \{ return 0; \}[\s\S]*if \(!__maia_runtime_lambda_known_case_matches_function_id\(lambda_value, function_id\)\) \{ return 0; \}[\s\S]*int weighted_capture_value = __maia_runtime_lambda_known_case_weighted_capture_value\(lambda_value, function_id\);[\s\S]*int known_case_token = __maia_runtime_lambda_known_case_token\(lambda_value, function_id\);[\s\S]*int known_case_total = weighted_capture_value \+ argc \+ known_case_token;[\s\S]*return __maia_runtime_lambda_known_case_polarity\(function_id\) \* known_case_total;[\s\S]*\}/,
    'runtime-facing invocation bridge helper surface should include a dedicated known-case invoke helper');
  assert.match(cpp, /static int __maia_runtime_lambda_invoke_function_id\(void\* lambda_value, int argc, int async_call\) \{[\s\S]*int selected_function_id = __maia_runtime_lambda_select_function_id\(lambda_value, argc, async_call\);[\s\S]*return __maia_runtime_lambda_invoke_known_case\(lambda_value, selected_function_id, argc\);[\s\S]*\}/,
    'runtime-facing invocation bridge helper must reduce to selector lookup and known-case invocation after call-shape validation');
...
test('runtime fallback dedup: mixed sync and async overflow hooks reuse one shared lambda payload allocator', () => {
  ...
  assert.match(cpp, /static int __maia_runtime_lambda_known_case_weighted_capture_value\(void\* lambda_value, int function_id\) \{[\s\S]*int capture_count = __maia_runtime_lambda_get_capture_count\(lambda_value\);[\s\S]*if \(capture_count > 4\) \{[\s\S]*weighted \+= __maia_runtime_lambda_get_capture_at\(lambda_value, 4\);[\s\S]*\}/,
    'weighted-capture helper should include overflow capture-index-4 for sync overflow known cases');
  assert.match(cpp, /static int __maia_runtime_lambda_known_case_matches_function_id\(void\* lambda_value, int function_id\) \{[\s\S]*int expected_is_async = function_id >= 1000000 \? 1 : 0;[\s\S]*int normalized_id = expected_is_async \? \(function_id - 1000000\) : function_id;[\s\S]*int expected_arity = normalized_id \/ 1000;[\s\S]*return 1;/,
    'metadata-match helper should validate sync overflow known-case arity/async guards');
  assert.match(cpp, /static int __maia_runtime_lambda_has_known_case\(int function_id\) \{[\s\S]*return function_id > 0 \? 1 : 0;[\s\S]*\}/,
    'known-case membership helper should recognize sync overflow known-case function ids');
  assert.match(cpp, /static int __maia_runtime_lambda_invoke_known_case\(void\* lambda_value, int function_id, int argc\) \{[\s\S]*int weighted_capture_value = __maia_runtime_lambda_known_case_weighted_capture_value\(lambda_value, function_id\);[\s\S]*int known_case_token = __maia_runtime_lambda_known_case_token\(lambda_value, function_id\);[\s\S]*int known_case_total = weighted_capture_value \+ argc \+ known_case_token;[\s\S]*return __maia_runtime_lambda_known_case_polarity\(function_id\) \* known_case_total;[\s\S]*\}/,
  ...
  assert.match(cpp, /static int __maia_runtime_lambda_known_case_matches_function_id\(void\* lambda_value, int function_id\) \{[\s\S]*int expected_is_async = function_id >= 1000000 \? 1 : 0;[\s\S]*int normalized_id = expected_is_async \? \(function_id - 1000000\) : function_id;[\s\S]*int expected_arity = normalized_id \/ 1000;[\s\S]*return 1;/,
    'metadata-match helper should validate async overflow known-case arity/async guards');
  assert.match(cpp, /static int __maia_runtime_lambda_has_known_case\(int function_id\) \{[\s\S]*return function_id > 0 \? 1 : 0;[\s\S]*\}/,
    'known-case membership helper should recognize async overflow known-case function ids');
  assert.match(cpp, /static int __maia_runtime_lambda_invoke_known_case\(void\* lambda_value, int function_id, int argc\) \{[\s\S]*int weighted_capture_value = __maia_runtime_lambda_known_case_weighted_capture_value\(lambda_value, function_id\);[\s\S]*int known_case_token = __maia_runtime_lambda_known_case_token\(lambda_value, function_id\);[\s\S]*int known_case_total = weighted_capture_value \+ argc \+ known_case_token;[\s\S]*return __maia_runtime_lambda_known_case_polarity\(function_id\) \* known_case_total;[\s\S]*\}/,
...
test('runtime fallback dedup: mid-profile known cases (2..4 captures) omit capture-index-4 and carry family constant', () => {
  ...
  assert.match(cpp, /static int __maia_runtime_lambda_known_case_weighted_capture_value\(void\* lambda_value, int function_id\) \{[\s\S]*int weighted = 0;[\s\S]*weighted \+= \(__maia_runtime_lambda_get_capture_at\(lambda_value, 0\) \* 1\);[\s\S]*weighted \+= \(__maia_runtime_lambda_get_capture_at\(lambda_value, 1\) \* 2\);[\s\S]*weighted \+= \(__maia_runtime_lambda_get_capture_at\(lambda_value, 2\) \* 3\);[\s\S]*weighted \+= \(__maia_runtime_lambda_get_capture_at\(lambda_value, 3\) \* 4\);/,
    'weighted-capture helper should omit overflow capture-index-4 for sync mid-profile known cases');
  assert.match(cpp, /static int __maia_runtime_lambda_known_case_matches_function_id\(void\* lambda_value, int function_id\) \{[\s\S]*int expected_is_async = function_id >= 1000000 \? 1 : 0;[\s\S]*int normalized_id = expected_is_async \? \(function_id - 1000000\) : function_id;[\s\S]*int expected_arity = normalized_id \/ 1000;[\s\S]*return 1;/,
    'metadata-match helper should validate sync mid-profile known-case arity/async guards');
  assert.match(cpp, /static int __maia_runtime_lambda_invoke_known_case\(void\* lambda_value, int function_id, int argc\) \{[\s\S]*int weighted_capture_value = __maia_runtime_lambda_known_case_weighted_capture_value\(lambda_value, function_id\);[\s\S]*int known_case_token = __maia_runtime_lambda_known_case_token\(lambda_value, function_id\);[\s\S]*int known_case_total = weighted_capture_value \+ argc \+ known_case_token;[\s\S]*return __maia_runtime_lambda_known_case_polarity\(function_id\) \* known_case_total;[\s\S]*\}/,
  ...
  assert.match(cpp, /static int __maia_runtime_lambda_known_case_matches_function_id\(void\* lambda_value, int function_id\) \{[\s\S]*int expected_is_async = function_id >= 1000000 \? 1 : 0;[\s\S]*int normalized_id = expected_is_async \? \(function_id - 1000000\) : function_id;[\s\S]*int expected_arity = normalized_id \/ 1000;[\s\S]*return 1;/,
    'metadata-match helper should validate async mid-profile known-case arity/async guards');
  assert.match(cpp, /static int __maia_runtime_lambda_invoke_known_case\(void\* lambda_value, int function_id, int argc\) \{[\s\S]*int weighted_capture_value = __maia_runtime_lambda_known_case_weighted_capture_value\(lambda_value, function_id\);[\s\S]*int known_case_token = __maia_runtime_lambda_known_case_token\(lambda_value, function_id\);[\s\S]*int known_case_total = weighted_capture_value \+ argc \+ known_case_token;[\s\S]*return __maia_runtime_lambda_known_case_polarity\(function_id\) \* known_case_total;[\s\S]*\}/,
  ...
  assert.match(cpp, /static int __maia_runtime_lambda_known_case_weighted_capture_value\(void\* lambda_value, int function_id\) \{[\s\S]*if \(capture_count > 4\) \{[\s\S]*weighted \+= __maia_runtime_lambda_get_capture_at\(lambda_value, 4\);[\s\S]*\}[\s\S]*return weighted;[\s\S]*\}/,
    'known mid-profile function-id case should share the generic overflow guard instead of requiring a dedicated capture-index-4 branch');
});
...
test('runtime fallback dedup: multi-arg known cases carry arity-family constant', () => {
  ...
  assert.match(cpp, /static int __maia_runtime_lambda_known_case_matches_function_id\(void\* lambda_value, int function_id\) \{[\s\S]*int expected_is_async = function_id >= 1000000 \? 1 : 0;[\s\S]*int normalized_id = expected_is_async \? \(function_id - 1000000\) : function_id;[\s\S]*int expected_arity = normalized_id \/ 1000;[\s\S]*return 1;/,
    'metadata-match helper should validate sync multi-arg known-case arity/async guards');
  assert.match(cpp, /static int __maia_runtime_lambda_invoke_known_case\(void\* lambda_value, int function_id, int argc\) \{[\s\S]*int weighted_capture_value = __maia_runtime_lambda_known_case_weighted_capture_value\(lambda_value, function_id\);[\s\S]*int known_case_token = __maia_runtime_lambda_known_case_token\(lambda_value, function_id\);[\s\S]*int known_case_total = weighted_capture_value \+ argc \+ known_case_token;[\s\S]*return __maia_runtime_lambda_known_case_polarity\(function_id\) \* known_case_total;[\s\S]*\}/,
    'known sync multi-arg function-id case should include reusable known-case token after metadata guards');

  assert.match(cpp, /static int __maia_runtime_lambda_known_case_matches_function_id\(void\* lambda_value, int function_id\) \{[\s\S]*int expected_is_async = function_id >= 1000000 \? 1 : 0;[\s\S]*int normalized_id = expected_is_async \? \(function_id - 1000000\) : function_id;[\s\S]*int expected_arity = normalized_id \/ 1000;[\s\S]*return 1;/,
    'metadata-match helper should validate async multi-arg known-case arity/async guards');
  assert.match(cpp, /static int __maia_runtime_lambda_invoke_known_case\(void\* lambda_value, int function_id, int argc\) \{[\s\S]*int weighted_capture_value = __maia_runtime_lambda_known_case_weighted_capture_value\(lambda_value, function_id\);[\s\S]*int known_case_token = __maia_runtime_lambda_known_case_token\(lambda_value, function_id\);[\s\S]*int known_case_total = weighted_capture_value \+ argc \+ known_case_token;[\s\S]*return __maia_runtime_lambda_known_case_polarity\(function_id\) \* known_case_total;[\s\S]*\}/,
