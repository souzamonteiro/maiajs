...
OUTPUT="$(node -e "globalThis.getResponse = () => Promise.resolve({ status: 201, meta: { status: 202 }, score: 3.5, makeMeta() { return { status: this.meta.status, nested: { status: 303, score: 4.25, label: 'inner' }, score: 3.5, label: 'ready' }; }, useMeta(meta) { return meta.status; }, describe(prefix) { return prefix + this.status; }, combine(prefix, meta, scale) { return prefix + (meta.status * scale); }, scale(value) { return this.status * value; }, count(a, b) { return a + b; } }); globalThis.getMessage = () => Promise.resolve('async dynamic string retained'); require(process.argv[1]);" "$TMP_DIR/dist/node-runner.js" 2>&1)"
printf '%s\n' "$OUTPUT"
...
grep -Fq 'async dynamic method nested object retained' <<<"$OUTPUT"
grep -Fq 'async dynamic method nested fractional property retained' <<<"$OUTPUT"
