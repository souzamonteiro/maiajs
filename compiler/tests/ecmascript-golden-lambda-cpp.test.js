...
test('golden: invocation bridge helpers validate call-shape and select function id', () => {
  ...
  assert.match(cpp, /static int __maia_runtime_lambda_known_case_token\(void\* lambda_value, int function_id\) \{[\s\S]*case 1001001:[\s\S]*return \(__maia_runtime_lambda_get_arity\(lambda_value\) \* 10\) \+ 1 \+ 10 \+ 100 \+ \(__maia_runtime_lambda_get_capture_count\(lambda_value\) \* 1000\);[\s\S]*default:[\s\S]*return 0;[\s\S]*\}/,
    'invocation bridge helpers should emit a dedicated known-case token helper keyed by function id');
  assert.match(cpp, /static int __maia_runtime_lambda_known_case_polarity\(int function_id\) \{[\s\S]*case 1001001:[\s\S]*return -1;[\s\S]*default:[\s\S]*return 0;[\s\S]*\}/,
    'invocation bridge helpers should emit a dedicated known-case polarity helper keyed by function id');
  assert.match(cpp, /static int __maia_runtime_lambda_known_case_weighted_capture_value\(void\* lambda_value, int function_id\) \{[\s\S]*case 1001001:[\s\S]*return \(__maia_runtime_lambda_get_capture_at\(lambda_value, 0\) \* 1\) \+ \(__maia_runtime_lambda_get_capture_at\(lambda_value, 1\) \* 2\) \+ \(__maia_runtime_lambda_get_capture_at\(lambda_value, 2\) \* 3\) \+ \(__maia_runtime_lambda_get_capture_at\(lambda_value, 3\) \* 4\);[\s\S]*default:[\s\S]*return 0;[\s\S]*\}/,
    'invocation bridge helpers should emit a dedicated weighted-capture helper keyed by function id');
  assert.match(cpp, /static int __maia_runtime_lambda_known_case_matches_function_id\(void\* lambda_value, int function_id\) \{[\s\S]*case 1001001:[\s\S]*if \(__maia_runtime_lambda_get_arity\(lambda_value\) != 1\) \{ return 0; \}[\s\S]*if \(__maia_runtime_lambda_get_is_async\(lambda_value\) != 1\) \{ return 0; \}[\s\S]*return 1;[\s\S]*default:[\s\S]*return 0;[\s\S]*\}/,
    'invocation bridge helpers should emit a dedicated known-case metadata-match helper keyed by function id');
  assert.match(cpp, /static int __maia_runtime_lambda_has_known_case\(int function_id\) \{[\s\S]*case 1001001:[\s\S]*return 1;[\s\S]*default:[\s\S]*return 0;[\s\S]*\}/,
  ...
  assert.match(cpp, /static int __maia_runtime_lambda_invoke_known_case\(void\* lambda_value, int function_id, int argc\) \{[\s\S]*if \(!__maia_runtime_lambda_has_known_case\(function_id\)\) \{ return 0; \}[\s\S]*int known_case_token = __maia_runtime_lambda_known_case_token\(lambda_value, function_id\);[\s\S]*if \(!known_case_token\) \{ return 0; \}[\s\S]*int known_case_polarity = __maia_runtime_lambda_known_case_polarity\(function_id\);[\s\S]*if \(!known_case_polarity\) \{ return 0; \}[\s\S]*int weighted_capture_value = __maia_runtime_lambda_known_case_weighted_capture_value\(lambda_value, function_id\);[\s\S]*if \(!__maia_runtime_lambda_known_case_matches_function_id\(lambda_value, function_id\)\) \{ return 0; \}[\s\S]*return known_case_polarity \* \(weighted_capture_value \+ argc \+ known_case_token\);[\s\S]*\}/,
    'invocation bridge helpers should emit a dedicated known-case invoke helper that derives membership, token, polarity, weighted-capture, and metadata-match values before returning the structured formula');
  assert.match(cpp, /static int __maia_runtime_lambda_invoke_function_id\(void\* lambda_value, int argc, int async_call\) \{[\s\S]*if \(!__maia_runtime_lambda_can_invoke\(lambda_value, argc, async_call\)\) \{ return 0; \}[\s\S]*int function_id = __maia_runtime_lambda_select_function_id\(lambda_value, argc, async_call\);[\s\S]*if \(!function_id\) \{ return 0; \}[\s\S]*return __maia_runtime_lambda_invoke_known_case\(lambda_value, function_id, argc\);[\s\S]*\}/,
    'invocation bridge dispatch helper should reduce to call-shape gating, selector lookup, and known-case invocation');
});
