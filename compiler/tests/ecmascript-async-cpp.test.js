...
test('async C++ emission: dynamic await handles lower string and object reads through the runtime ABI', () => {
  const cpp = runCompilerCpp(
    'async function load() { const response = await getResponse(); if (response.status === 201) { console.log("ok"); } if (response.meta.status === 202) { console.log("nested"); } if (response.makeMeta().status === 202) { console.log("method object"); } if (response.makeMeta().nested.status === 303) { console.log("method nested object"); } if (response.makeMeta().nested.score === 4.25) { console.log("method nested fractional"); } if (response.makeMeta().nested.label === "inner") { console.log("method nested string"); } if (response.useMeta(response.makeMeta()) === 202) { console.log("method object argument"); } if (response.useMeta(response.makeTaggedMeta("ready")) === 202) { console.log("method object argument with scalar"); } if (response.sumValues([2, 3]) === 5) { console.log("array argument"); } if (response.readMeta({ status: 202, label: "ready" }) === 202) { console.log("object argument"); } if (response.makeMeta().score === 3.5) { console.log("method object fractional"); } if (response.makeMeta().label === "ready") { console.log("method object string"); } console.log(response.makeMeta().label); console.log(response.makeMeta().score); console.log(response.makeMeta().nested.status); if (response.score === 3.5) { console.log("fractional"); } if (response.describe("status: ") === "status: 201") { console.log("method string"); } if (response.scale(2.5) === 502.5) { console.log("method fractional"); } if (response.count(2, 3) === 5) { console.log("method integer"); } const description = response.describe("status: "); const combined = response.combine("meta: ", response.meta, 2.5); console.log(description); console.log(combined); console.log(response.scale(2.5)); const message = await getMessage(); console.log(message); }\n'
  );
  ...
  assert.match(cpp, /__async_handle_get_i32\(__async_handle_call0\(__sm->__local_response, \(const char\*\)"makeMeta"\), \(const char\*\)"status"\) == 202/, 'object-returning dynamic methods must feed a following property access through a handle');
  assert.match(cpp, /__async_handle_get_i32\(__async_handle_get_handle\(__async_handle_call0\(__sm->__local_response, \(const char\*\)"makeMeta"\), \(const char\*\)"nested"\), \(const char\*\)"status"\) == 303/, 'nested properties returned by dynamic methods must retain intermediate handles');
  assert.match(cpp, /__async_handle_get_f64\(__async_handle_get_handle\(__async_handle_call0\(__sm->__local_response, \(const char\*\)"makeMeta"\), \(const char\*\)"nested"\), \(const char\*\)"score"\) == 4\.25/, 'nested fractional properties must select the f64 getter');
  assert.match(cpp, /__async_handle_equals_string\(__async_handle_get_handle\(__async_handle_get_handle\(__async_handle_call0\(__sm->__local_response, \(const char\*\)"makeMeta"\), \(const char\*\)"nested"\), \(const char\*\)"label"\), \(const char\*\)\("inner"\)\)/, 'nested string properties must preserve strict equality');
  assert.match(cpp, /__async_handle_to_i32\(\(__async_handle_arg_handle\(__async_handle_call0\(__sm->__local_response, \(const char\*\)"makeMeta"\)\), __async_handle_callN\(__sm->__local_response, \(const char\*\)"useMeta", 1\)\)\) == 202/, 'object-returning dynamic methods must be usable as dynamic method arguments');
  assert.match(cpp, /__async_handle_to_i32\(\(__async_handle_arg_handle\(__async_handle_call1_string\(__sm->__local_response, \(const char\*\)"makeTaggedMeta", \(const char\*\)\("ready"\)\)\), __async_handle_callN\(__sm->__local_response, \(const char\*\)"useMeta", 1\)\)\) == 202/, 'dynamic method arguments must retain an object result from an inner scalar call');
  assert.match(cpp, /__async_handle_aggregate_begin_array\(\);\s+__async_handle_aggregate_push_i32\(2\);\s+__async_handle_aggregate_push_i32\(3\);\s+int __async_aggregate_tmp\d+ = __async_handle_aggregate_end\(\);/, 'array literals must be staged into a host aggregate temporary');
  assert.match(cpp, /__async_handle_aggregate_begin_object\(\);\s+__async_handle_aggregate_set_i32\(\(const char\*\)"status", 202\);\s+__async_handle_aggregate_set_string\(\(const char\*\)"label", \(const char\*\)\("ready"\)\);\s+int __async_aggregate_tmp\d+ = __async_handle_aggregate_end\(\);/, 'object literals must preserve typed keys and values in the host aggregate ABI');
  assert.match(cpp, /__async_handle_get_f64\(__async_handle_call0\(__sm->__local_response, \(const char\*\)"makeMeta"\), \(const char\*\)"score"\) == 3\.5/, 'fractional properties returned by dynamic methods must select the f64 getter');
  assert.match(cpp, /__async_handle_equals_string\(__async_handle_get_handle\(__async_handle_call0\(__sm->__local_response, \(const char\*\)"makeMeta"\), \(const char\*\)"label"\), \(const char\*\)\("ready"\)\)/, 'string properties returned by dynamic methods must preserve strict equality');
  assert.match(cpp, /__console__log\(__async_handle_get_property_string\(__async_handle_call0\(__sm->__local_response, \(const char\*\)"makeMeta"\), \(const char\*\)"label"\)\);/, 'console output must consume the string property rather than its object receiver');
  assert.match(cpp, /__console__log\(__async_handle_get_property_string\(__async_handle_call0\(__sm->__local_response, \(const char\*\)"makeMeta"\), \(const char\*\)"score"\)\);/, 'console output must format a numeric property rather than dropping its handle');
  assert.match(cpp, /__console__log\(__async_handle_get_property_string\(__async_handle_get_handle\(__async_handle_call0\(__sm->__local_response, \(const char\*\)"makeMeta"\), \(const char\*\)"nested"\), \(const char\*\)"status"\)\);/, 'console output must follow nested handles to the final property');
