...
test('array literal lowering: emits runtime hook declaration and empty array construction', () => {
  ...
  assert.match(cpp, /void\* arr = __maia_arr_literal0\(\);/, 'C++ must initialize the array through its runtime literal helper');
...
test('array literal lowering: lowers non-empty array with literal elements', () => {
  ...
  assert.match(cpp, /extern void\* __maia_arr_literal2\(long v1, long v2\);/, 'C++ must declare the arity-2 array runtime hook');
  assert.match(cpp, /void\* arr = __maia_arr_literal2\(\(long\)\(1\), \(long\)\(2\)\);/, 'C++ must preserve non-empty array element values');
});

test('array literal lowering: lowers identifier element expressions', () => {
  const cpp = runCompilerCpp('let x = 7;\nlet arr = [x];\n');

  assert.match(cpp, /double x = 7;/, 'C++ must preserve the declaration used by the array literal');
  assert.match(cpp, /void\* arr = __maia_arr_literal1\(\(long\)\(x\)\);/, 'C++ must preserve identifier elements in the array literal');
});
...
test('array literal lowering: falls back for unsupported arity > 4', () => {
  ...
  assert.match(cpp, /void\* arr = __maia_arr_builder_end\(/, 'C++ must finish large arrays through the runtime builder');
...
test('array literal lowering: falls back for spread elements in array literal', () => {
  ...
  assert.match(cpp, /void\* arr = __maia_arr_builder_end\(/, 'C++ must finish spread arrays through the runtime builder');
...
test('array literal lowering: falls back for elision forms in array literal', () => {
  ...
  assert.match(cpp, /void\* arr = __maia_arr_builder_end\(/, 'C++ must finish elision arrays through the runtime builder');
