...
test('async C++ emission: dynamic await handles lower string and object reads through the runtime ABI', () => {
  ...
  assert.match(cpp, /__console__log\(__async_handle_get_string\(__async_handle_get_handle\(__async_handle_call0\(__sm->__local_response, \(const char\*\)"makeMeta"\), \(const char\*\)"label"\)\)\);/, 'console output must consume the string property rather than its object receiver');
