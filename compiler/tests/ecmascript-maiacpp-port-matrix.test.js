...
test('MaiaCpp port matrix: classes, arrays, objects, and new expressions stay coherent', () => {
  ...
  assert.match(cpp, /void\* arr = __maia_arr_literal2\(\(long\)\(1\), \(long\)\(2\)\);/, 'array literal must preserve its element values');
  assert.match(cpp, /__maia_obj_literal2\(\(char\*\)"a", \(long\)\(1\), \(char\*\)"b", \(long\)\(2\)\);/, 'object literal must emit the arity-based helper');
  assert.match(cpp, /Point __maia_new_Point_0;/, 'standalone construction must allocate a valid C++98 temporary');
  assert.match(cpp, /Point_ctor_init__pvii\(\(Point\*\)&__maia_new_Point_0, 1, 2\);/, 'standalone construction must invoke the Maia constructor wrapper');
