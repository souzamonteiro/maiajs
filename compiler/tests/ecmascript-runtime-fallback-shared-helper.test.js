...
test('runtime fallback dedup: capture-aware lambda fallback reuses shared lambda payload allocator', () => {
  ...
  assert.match(cpp, /switch \(function_id\) \{[\s\S]*case 1001:[\s\S]*if \(__maia_runtime_lambda_get_arity\(lambda_value\) != 1\) \{ return 0; \}[\s\S]*if \(__maia_runtime_lambda_get_is_async\(lambda_value\) != 0\) \{ return 0; \}[\s\S]*return __maia_runtime_lambda_get_capture_at\(lambda_value, 0\) \+ __maia_runtime_lambda_get_capture_at\(lambda_value, 1\) \+ argc;/,
    'runtime-facing invocation bridge helper should materialize known case labels with metadata guards and second-capture argc-aware callable-like sync return behavior');
...
test('runtime fallback dedup: mixed sync and async overflow hooks reuse one shared lambda payload allocator', () => {
  ...
  assert.match(cpp, /case 1005:[\s\S]*if \(__maia_runtime_lambda_get_arity\(lambda_value\) != 1\) \{ return 0; \}[\s\S]*if \(__maia_runtime_lambda_get_is_async\(lambda_value\) != 0\) \{ return 0; \}[\s\S]*return __maia_runtime_lambda_get_capture_at\(lambda_value, 0\) \+ __maia_runtime_lambda_get_capture_at\(lambda_value, 1\) \+ argc;/,
    'known sync function-id case should return second-capture argc-aware callable-like value after metadata guards');

  assert.match(cpp, /case 1001005:[\s\S]*if \(__maia_runtime_lambda_get_arity\(lambda_value\) != 1\) \{ return 0; \}[\s\S]*if \(__maia_runtime_lambda_get_is_async\(lambda_value\) != 1\) \{ return 0; \}[\s\S]*return -\(__maia_runtime_lambda_get_capture_at\(lambda_value, 0\) \+ __maia_runtime_lambda_get_capture_at\(lambda_value, 1\) \+ argc\);/,
    'known async function-id case should return async-specific second-capture argc-aware callable-like value after metadata guards');
});
