...
OUTPUT="$(node -e "globalThis.tick = () => Promise.resolve(0); globalThis.tickAgain = () => Promise.resolve(0); require(process.argv[1]);" "$TMP_DIR/dist/node-runner.js" 2>&1)"
printf '%s\n' "$OUTPUT"
test "$(grep -Fxc 'async while tick' <<<"$OUTPUT")" -eq 2
test "$(grep -Fxc 'async while second tick' <<<"$OUTPUT")" -eq 2
