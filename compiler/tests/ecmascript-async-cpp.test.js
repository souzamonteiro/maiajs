...
test('async C++ emission: dynamic await handles lower string and object reads through the runtime ABI', () => {
  ...
  assert.match(cpp, /__sm->__local_methodMeta = __async_handle_call0\(__sm->__local_response, \(const char\*\)"makeMeta"\);/, 'object-returning dynamic methods must remain handles when bound');
  assert.match(cpp, /__async_handle_get_i32\(__sm->__local_methodMeta, \(const char\*\)"status"\) == 202/, 'a bound object-returning method result must support property access through its handle');
